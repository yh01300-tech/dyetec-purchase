...
if menu == "종합 대시보드":
    ...
    if not df.empty and '매입일자' in df.columns:
        ...
        st.subheader("🏆 거래처별 매입 비중 (이번 달)")
        if not curr.empty: st.bar_chart(curr.groupby('거래처')['총액'].sum())
    else: st.info("매입 자료가 비어있습니다. 데이터를 입력해주세요.")

elif menu == "단가 검색":
    st.title("🔎 품목별 최신 단가 검색")
    df_h = load_data("단가이력")
    if not df_h.empty:
        item = st.selectbox("조회할 품목 선택", df_h['품목명'].unique())
        hist = df_h[df_h['품목명'] == item].sort_values('변경일자', ascending=False)
        st.write(f"현재 최신 단가: {int(hist.iloc[0]['단가']):,} 원 / 변동일: {hist.iloc[0]['변경일자']}")
    ...
    if sub:
        ...
        st.success("등록 완료되었습니다!"); st.rerun()
    st.subheader("최근 입력된 내역")
    st.dataframe(load_data("매입자료").tail(10), use_container_width=True)

elif menu == "거래처 등록":
    st.title("🏢 거래처 등록 및 정보 수정")
    df_v = load_data("거래처")
    action = st.radio("작업 선택", ("신규 등록", "정보 수정"), horizontal=True)
    d = {}
    ...
    with st.form("v_form", clear_on_submit=True):
        c1, c2 = st.columns(2)
        n = c1.text_input("거래처명", value=d.get('거래처명', ''))
        biz = c1.text_input("사업자번호", value=d.get('사업자등록번호', ''))
        p1 = c1.text_input("연락처1", value=d.get('연락처1', ''))
        p2 = c2.text_input("연락처2", value=d.get('연락처2', ''))
        fax = c2.text_input("팩스번호", value=d.get('팩스번호', ''))
        addr = c2.text_input("주소", value=d.get('주소', ''))
        rem = st.text_input("비고", value=d.get('비고', ''))
    ...
    with st.form("i_form", clear_on_submit=True):
        ...
        p = st.number_input("단가", value=int(d.get('단가', 0)))
        sub = st.form_submit_button("저장")
    if sub:
    ...
    if not df.empty:
        c1, c2 = st.columns(2)
        v_list = ["전체"] + df['거래처'].unique().tolist()
        i_list = ["전체"] + df['품목명'].unique().tolist()
        v = c1.selectbox("거래처 선택", v_list)
        i = c2.selectbox("품목 선택", i_list)
    ...
    if not df.empty and '매입일자' in df.columns:
        df['매입일자_dt'] = pd.to_datetime(df['매입일자'], errors='coerce')
        ym_list = sorted(df['매입일자_dt'].dt.strftime('%Y-%m').unique().tolist(), reverse=True)
        sel_ym = st.selectbox("월 선택", ym_list)
        sel_v = st.selectbox("거래처 선택", df['거래처'].unique().tolist())
        filtered = df[(df['매입일자_dt'].dt.strftime('%Y-%m') == sel_ym) & (df['거래처'] == sel_v)]
        st.dataframe(filtered, use_container_width=True)
        st.info(f"합계 금액: {int(filtered['총액'].sum()):,} 원")
