...
st.title("현대다이텍 시스템") # 앱 상단 제목 복구
conn = st.connection("gsheets", type=GSheetsConnection)

# 2. 인쇄 최적화 CSS (화면 UI는 유지하되, 인쇄할 때만 버튼/입력창 숨김)
st.markdown("""
    <style>
    @media print {
        [data-testid="stSidebar"], .stButton, .stTextInput, .stSelectbox, .stDateInput, .stNumberInput, .stForm, .stRadio {
...
if menu == "종합 대시보드":
    ...
    if not df.empty and '매입일자' in df.columns:
        ...
        # 인쇄 영역
        st.markdown(f"<h2>[{sel_v}] {sel_ym}월 매입 정산서</h2>", unsafe_allow_html=True)
        st.dataframe(filtered, use_container_width=True)
        st.write(f"<h3>💰 합계 금액: {int(filtered['총액'].sum()):,} 원</h3>", unsafe_allow_html=True)
        st.info("💡 인쇄(Ctrl+P) 시 UI는 사라지고 정산서 양식만 출력됩니다.")
