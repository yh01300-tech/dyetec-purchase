...
if menu == "종합 대시보드":
    ...
    if not df.empty:
        ...
        if '거래처' in df.columns: st.bar_chart(curr.groupby('거래처')['총액'].sum())
    ...
    if sub:
        if action == "신규 등록":
            conn.update("거래처", pd.concat([df_v, pd.DataFrame([{"거래처명":n, "사업자등록번호":biz, "연락처1":p1, "연락처2":p2, "팩스번호":fax, "주소":addr, "비고":rem}])], ignore_index=True))
        else:
